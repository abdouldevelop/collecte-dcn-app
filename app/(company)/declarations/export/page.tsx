...
export const metadata: Metadata = { title: "Déclaration Export — Collecte DCN" };
...
export default async function ExportDeclarationPage() {
  ...
  const declaration = await prisma.exportDeclaration.findUnique({
  ...
  const companyProducts = await prisma.companyProduct.findMany({
    where: {
      ...
      isActive: true,
      product: { type: { in: ["EXPORT", "IMPORT_EXPORT"] } },
    },
